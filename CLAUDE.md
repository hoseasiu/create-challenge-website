...
**Wording must be preserved verbatim, with objective-error corrections as the one exception (learned 2026-08-19, refined 2026-08-19).** When converting a page's body copy — headings, paragraphs, list items, link text, captions, everything a visitor reads — carry the source text over exactly as written. Restructuring into this site's HTML components (prose blocks, lists, reflect-boxes, video-stacks) is fine and expected; changing, condensing, paraphrasing, or dropping any of the actual words is not, and neither is adding sentences/claims that aren't in the source, even if they sound plausible or fill an obvious gap — including a `TODO` placeholder: keep it literally as `TODO`, don't "finish" it into prose.

**Exception — fix-and-log objective errors.** Genuine mistakes in the source may be corrected without asking first, as long as the fix is unambiguous (one clearly-correct reading, meaning unchanged) rather than a rewrite or a style/quality judgment call. In scope for silent-but-logged fixes:
- Misspellings, duplicated words, wrong-word slips (e.g. "willing the pay" → "willing to pay"), missing small function words, stray whitespace/punctuation.
- Grammar fixes, even ones touching more than one word, as long as they're unambiguous corrections rather than rewording (e.g. "or to help with decide what clothes to wear" → "or to help decide what clothes to wear").
- Misspelled or miscapitalized proper nouns/brand names where there's one objectively correct form (e.g. "Blufruit" → "Bluefruit," "Sparkfun" → "SparkFun," "Beagle Bone" → "BeagleBone").
- Non-descriptive link text (literal "here" links) — rewriting it to be descriptive counts as an accessibility correction, not a wording change subject to the verbatim rule.
- A link's `href` pointing somewhere stale/broken when its visible text implies a different destination — redirect the `href` to match what the text says; this is a technical fix, not a wording change.

Out of scope — still ask first, don't decide unilaterally: condensing, paraphrasing, dropping or adding sentences, "finishing" placeholder text, rewording anything that isn't a clear objective error, and any change to outdated/broken *content* (as opposed to a typo) like stale program names or dates.

When you make one of these in-scope corrections, don't just fix it silently — log it. A short "was → fixed to" note in your response (or the PR/commit description) is enough for routine conversions; reserve a dedicated standalone audit file (like [maker-skills-verbatim-audit.md](maker-skills-verbatim-audit.md)) for large remediations, not routine single-page conversions.

This applies to every full HTML-embed conversion, not just Maker Skills — the 2026-08-15 Maker Skills sub-pages were built from 4 parallel subagents that all reworded/condensed the source text while logging it as "verbatim," and it took a manual audit against the hidden originals to catch it (see [maker-skills-verbatim-audit.md](maker-skills-verbatim-audit.md) for the full before/after, including the 2026-08-19 follow-up pass that first established this fix-and-log pattern for genuine errors).
...
7. **Before considering the conversion done, check the new HTML against the source page one paragraph/list-item/link at a time** — don't just assert the content is verbatim; actually re-read source and output side by side. For anything reworded, condensed, dropped, or added that isn't an in-scope objective-error fix (see the exception above), flag it to Hosea rather than silently fixing or silently declaring it clean. For anything that *is* in scope (typo, unambiguous grammar slip, misspelled brand name, non-descriptive link text, stale `href`), fix it and log it — "was → fixed to" — rather than leaving it in place or just flagging it.
